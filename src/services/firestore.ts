import { 
  ...
  export const rateMember = async (
    ...
    const ratingId = `${teamId}_${raterId}_${ratedUserId}`;
  ...
  export const addTeamMember = async (teamId: string, userId: string, role: string): Promise<void> => {
   ...
   const userName = profile?.username || profile?.fullName || "User";
    ...
    // Add member to team's members array (email stored for notifications)
    const updatedMembers = [...(team.members || []), { userId, role, userName, email: profile?.email || null }];
  ...
  const members = await Promise.all(
    (team.members || []).map(async (member) => {
      ...
      return {
        ...
        role: member.role,
        userName: member.userName || profile?.username || profile?.fullName || "User",
        email: profile?.email || null,   // ✅ Registration email for notifications
...
if (isJoinRequest) {
    const { allowed } = await chargeJoinFee(data.fromUserId);
    if (!allowed) {
      throw new Error(
        'You need at least 50 Perks to send a join request. Earn more by completing tasks!'
      );
    }
  }

  // ✅ Create invitation/join request (Only happens if charge was successful)
  ...
  export const respondToInvitation = async (
    ...
    if (status === 'accepted') {
      ...
      if (!isJoinRequest) {
        const { allowed } = await chargeJoinFee(joiningUserId);
        if (!allowed) {
          throw new Error(
            'You need at least 50 Perks to accept this invitation. Earn more by completing tasks!'
          );
        }
      }
  ...
  export const unsubmitTask = async (taskId: string): Promise<void> => {
    if (!isFirebaseConfigured()) return;
  ...
  // Submit proof for a task (member side)
  export const submitTaskProof = async (
    taskId: string,
    submittedBy: string,
    proof: { type: 'file' | 'link' | 'text'; value: string; fileName?: string }
  ): Promise<void> => {
    if (!isFirebaseConfigured()) return;
    await updateDoc(doc(db, 'teamTasks', taskId), {
      status: 'submitted',
  ...
  export const updateTeamTimeline = async (
    teamId: string,
    stages: { id: string; label: string; completed: boolean }[]
  ): Promise<void> => {
    if (!isFirebaseConfigured()) return;
    await updateDoc(doc(db, 'teams', teamId), { timeline: stages });
  };
  // ========================
  // STATS FUNCTIONS
  // ========================
