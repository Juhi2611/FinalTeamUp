  ...
  const [ratings, setRatings] = useState<Record<string, number>>({});
    ...
    const fetchRatings = async () => {
      ...
      for (const member of team.loadedMembers) {
        ...
        if (snap.exists()) {
          newRatings[member.userId] = snap.data().rating;
        }
      }
...
if (editingTeamId) {
                ...
                <div className="relative">
                  ...
                  {openMenu === team.id && (
                    <div className="absolute right-0 mt-2 w-56 bg-card border border-border rounded-xl shadow-lg z-50 overflow-hidden">
                      ...
                      {/* Rate Members (Leader + completed) */}
                      {isLeader && isCompleted && (
                        <button
                          onClick={() => {
                            setOpenMenu(null);
                            setShowRatingTeam(team.id);
                          }}
                          className="menu-item"
                        >
                          ⭐ Rate Members
  ...
  member.profile?.avatar
    ? `${member.profile.avatar}?t=${
        member.profile.updatedAt instanceof Timestamp
          ? member.profile.updatedAt.toMillis()
          : ''
      }`
    : `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(
        member.profile?.fullName || 'User'
              ...
              <div className="mb-4 p-4 rounded-xl bg-secondary/20 border border-border">
                <ProjectTimeline
                  stages={((team as any).timeline as ProjectStage[])?.length ? (team as any).timeline : DEFAULT_STAGES}
                  teamId={team.id}  
                  isLeader={isLeader}
                  onSave={async (stages: ProjectStage[]) => {
                    try {
                      await updateTeamTimeline(team.id, stages);
                                  ...
                                  {recommendationsByTeam[team.id].recommendedUsers.map((rec, idx) => (
                                    ...
                                    <img
                                      src={
                                        rec.user.avatar
                                          ? `${rec.user.avatar}?t=${
                                              rec.user.updatedAt instanceof Timestamp
                                                ? rec.user.updatedAt.toMillis()
                                                : ''
                                            }`
                                          : `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(
      ...
      {showRatingTeam && (
        <div className="modal-overlay" onClick={() => setShowRatingTeam(null)}>
          <div
            className="bg-card rounded-2xl p-6 w-full max-w-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="font-bold text-lg mb-4">Rate Team Members</h2>
            ...
            {(() => {
              ...
              return (
                <div className="space-y-4">
                  {team.loadedMembers.map(member => {
                    if (member.userId === user?.uid) return null; // no self rating
                    ...
                    return (
                      <div key={member.id} className="flex items-center justify-between">
                        ...
                        <StarRating
                          rating={ratings[member.userId] || 0}
                          onChange={(val) => submitRating(member.userId, val, team.id)}
                          size="sm"
                        />
                      </div>
                    );
